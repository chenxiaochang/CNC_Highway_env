...
class testvehicleEnv(AbstractEnv):
    """
    ...
    def default_config(cls) -> dict:
        ...
        config.update({
            ...
            "other_vehicles_type1": "highway_env.vehicle.behavior.DefensiveVehicle",
            "other_vehicles_type2": "highway_env.vehicle.behavior.AggressiveVehicle",
    ...
    def _reward(self, action: int) -> float:
        ...
        if (self.steps >= (self.config["duration"]) or not self.vehicle.on_road):
            ...
            print("达到持续时间:{}".format(reward))
            print("on_road1:{}".format(self.vehicle.on_road))
        else:
            reward = self.COLLISION_REWARD * self.vehicle.crashed + self.HIGH_SPEED_REWARD * self.vehicle.speed_index / (
                    self.vehicle.SPEED_COUNT - 1) \
                     + self.LEFT_LANE_REWARD * (len(neighbours) - 1 - self.vehicle.target_lane_index[2]) / (
                             len(neighbours) - 1) + self.RIGHT_LANE_REWARD * (self.vehicle.target_lane_index[2]) / (
                             len(neighbours) - 1)
            print("还没呢:{}".format(reward))
            print("on_road2:{}".format(self.vehicle.on_road))
    ...
    def _make_vehicles(self) -> None:  # 道路车辆填充
        ...
        ego_vehicle = self.action_type.vehicle_class(road,
                                                     position=road.network.get_lane(("a", "b", 1)).position(30, 0),
                                                     heading=road.network.get_lane(("a", "b", 1)).heading_at(30),
                                                     speed=25)  # speed=30
        ...
        vehicles_type1 = utils.class_from_path(self.config["other_vehicles_type1"])
        vehicles_type2 = utils.class_from_path(self.config["other_vehicles_type2"])
        ...
        #                   .position(50, 0),
        #                   # .position(110 + self.np_random.randint(-15, 16), 0),
        #                   heading=road.network.get_lane(("a", "b", 1)).heading_at(110),
        #                   speed=30,
        #                   # speed=15 + self.np_random.randint(-3, 6),
        #                   enable_lane_change=False)
        # )
        #
        # self.road.vehicles.append(
        ...
        #                    position=road.network.get_lane(("a", "b", 1))
        #                    .position(100, 0),
        #                    # .position(180 + self.np_random.randint(-15, 16), 0),
        #                    heading=road.network.get_lane(("a", "b", 1)).heading_at(200),
        #                    speed=25,
        #                    # speed=26 + self.np_random.randint(-3, 6),
        #                    enable_lane_change=False)
        # )
        #
        # self.road.vehicles.append(
        ...
        #                    position=road.network.get_lane(("a", "b", 1))
        #                    .position(150, 0),
        #                    # .position(250 + self.np_random.randint(-15, 16), 0),
        #                    heading=road.network.get_lane(("a", "b", 1)).heading_at(290),
        #                    speed=30,
        #                    # speed=24 + self.np_random.randint(-3, 6),
        #                    enable_lane_change=False)
        # )
        # self.road.vehicles.append(
        #     vehicles_type(road,
        ...
        #     self.road.vehicles.append(
        #         vehicles_type(road,
        #                       position=road.network.get_lane(( ))  # 得到车道索引
        #                       .position(70 + 40 * i + 10 * self.np_random.randn(), 0),  # 车所在的位置
        #                       heading=road.network.get_lane(("a","b", 1)).heading_at(70 + 40 * i),
        #                       speed=24 + 2 * self.np_random.randn(),  # 每一辆车的速度
        #                       enable_lane_change=False)
        ...
        """ 在本车到上相向而行的两辆车，v1向左，v2向右，发生碰撞"""
        v1 = vehicles_type(road,
                           position=road.network.get_lane(("b", "a", 1))
                           .position(400, 0),
                           heading=road.network.get_lane(("b", "a", 1)).heading_at(400),
                           speed=20,
                           enable_lane_change=False)
        v1.target_lane_index = ("b", "a", 1)
        self.road.vehicles.append(v1)

        v2 = vehicles_type(road,
                           position=road.network.get_lane(("a", "b", 1))
                           .position(200, 0),
                           heading=road.network.get_lane(("a", "b", 1)).heading_at(200),
                           speed=20,
                           enable_lane_change=False)
        v2.target_lane_index = ("a", "b", 1)
        self.road.vehicles.append(v2)
        ...
        #                    .position(110 + self.np_random.randint(-15, 16), 0),
        #                    heading=road.network.get_lane(("b", "a", 0)).heading_at(400),
        #                    speed=15 + self.np_random.randint(-3, 6),
        #                    enable_lane_change=False)
        # v1.target_lane_index = ("b", "a", 0)
        # self.road.vehicles.append(v1)
        #
        # v2 = vehicles_type2(road,
        #                     position=road.network.get_lane(("b", "a", 0))
        #                     .position(180 + self.np_random.randint(-15, 16), 0),
        #                     heading=road.network.get_lane(("b", "a", 0)).heading_at(200),
        #                     speed=26 + self.np_random.randint(-3, 6),
        #                     enable_lane_change=False)
        # v2.target_lane_index = ("b", "a", 0)
        # self.road.vehicles.append(v2)
        #
        # v3 = vehicles_type(road,
        #                    position=road.network.get_lane(("b", "a", 0))
        #                    .position(460, 0),
        #                    # .position(250 + self.np_random.randint(-15, 16), 0),
        #                    heading=road.network.get_lane(("b", "a", 0)).heading_at(460),
        #                    speed=20,
        #                    # speed=24 + self.np_random.randint(-3, 6),
        #                    enable_lane_change=False)
        # v3.target_lane_index = ("b", "a", 0)
        # self.road.vehicles.append(v3)
        #
        # v4 = vehicles_type(road,
        #                    position=road.network.get_lane(("b", "a", 0))
        #                    .position(520, 0),
        #                    # .position(250 + self.np_random.randint(-15, 16), 0),
        #                    heading=road.network.get_lane(("b", "a", 0)).heading_at(520),
        #                    speed=15,
        #                    # speed=24 + self.np_random.randint(-3, 6),
        #                    enable_lane_change=False)
        # v4.target_lane_index = ("b", "a", 0)
        # self.road.vehicles.append(v4)

    # for i in range(5):  # 在车道0上加上3辆车
    #     v = vehicles_type(road,
    #                       position=road.network.get_lane(("b", "a", 0))
    #                       .position(200 + 80*i + self.np_random.randint(-15, 16), 0),
    #                       # .position(50 + 80 * i + 10 * self.np_random.randn(), 0),
...
register(
    id='testvehicle-v0',
    entry_point='highway_env.envs:testvehicleEnv',
    # max_episode_steps=4
)
